...
impl TestMsg {
  #[new]
  pub fn __new__( large_number: u32, lala: bool, lili: bool,) -> Self {
    Self{ large_number: large_number.into(), lala: lala.into(), lili: lili.into(), }
  }
  pub fn serialize(&self, py: Python) -> PyObject {
    let msg = self.to_rust();
    PyBytes::new(py, &serialize(&msg)).into()
  ...
  pub fn deserialize(_py: Python, data: Bound<'_, PyBytes>) -> PyResult<Self> {
    let dv: Vec<u8> = data.extract()?;
    let v = match deserialize::<messages::TestMsg>(&dv) {
      Some(v) => v, None => return Err(PyErr::new::<PyException, _>("Error when deserializing TestMsg"))
    };
    Self::from_rust_obj(v.0)
  }
  pub fn __repr__(&self) -> String {
...
impl TestMsg {
  pub fn to_rust(&self) -> messages::TestMsg {
    Python::with_gil(|_py| {
      messages::TestMsg{ large_number: self.large_number.clone().into(), lala: self.lala.clone().into(), lili: self.lili.clone().into(),}
    })
  }
  pub fn from_rust_obj(d: messages::TestMsg) -> PyResult<Self> {
    let r = Python::with_gil(|_py| { Self{ large_number: d.large_number.val, lala: d.lala, lili: d.lili,} });
    Ok(r)
  }
}
impl std::fmt::Display for TestMsg {
  fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
    Python::with_gil(|_py| {
      write!(f, "TestMsg(large_number: ({}), lala: ({}), lili: ({}), )", self.large_number, self.lala, self.lili,)
    })
  }
}
